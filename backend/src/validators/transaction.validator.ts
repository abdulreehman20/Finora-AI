...
export const baseTransactionSchema = z.object({
	...
	type: z.enum(
		[TransactionTypeEnum.INCOME, TransactionTypeEnum.EXPENSE] as const,
...
export const bulkDeleteTransactionSchema = z.object({
	transactionIds: z
		.array(z.string().uuid()) // or .length(36, "Invalid transaction ID format")
		...
		.refine(
			(txs) =>
				txs.every((tx) => {
					...
					return !Number.isNaN(amount) && amount > 0 && amount <= 1_000_000_000;
				}),
...
export type BulkTransactionItemType = z.infer<typeof bulkTransactionItemSchema>;
