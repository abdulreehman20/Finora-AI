...
import type { TransactionTypeEnum } from "../db/schema/transaction.schema.js";
...
export const bulkTransactionController = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!isSubscribed) {
      return res.status(HTTPSTATUS.FORBIDDEN).json({
        message:
          "Bulk import is only available on the Pro plan. Please upgrade to access this feature.",
