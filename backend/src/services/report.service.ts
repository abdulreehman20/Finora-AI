...
export const generateReportService = async (
  ...
  for (const tx of transactions) {
    ...
    if (tx.type === TransactionTypeEnum.INCOME) {
      ...
      categoryMap[tx.categoryName] = (categoryMap[tx.categoryName] ?? 0) + amount;
    }
  }
...
export const resendReportService = async (userId: string, reportId: string) => {
  const { APIError } = await import("../lib/apiError.js");
  const { HTTPSTATUS } = await import("../configs/http.config.js");
