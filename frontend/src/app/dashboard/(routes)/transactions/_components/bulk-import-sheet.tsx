  ...
  function validateAndProceed() {
    ...
    csvRows.forEach((row, i) => {
      ...
      mappings.forEach((m, j) => {
        ...
        if (m.field === "Category" && val) {
          const exists = categories.some(
            (c) => c.name.toLowerCase() === val.toLowerCase(),
          );
          if (!exists) {
            errs.push(`category: Unknown category "${val}"`);
          }
        }
      });
      if (errs.length > 0) {
    ...
    try {
      const transactions = csvRows
        .map((row) => {
          ...
          const categoryName = getCellValue(row, "Category");
          const category =
            categoryName ||
            categories.find((c) => c.type === type)?.name ||
            "";
          if (!category) {
            throw new Error(`Row missing a valid category`);
          }
